...
export function PgButton({
  ...
  const variants = {
    primary: "bg-signal-cyan text-void hover:opacity-90",
    secondary:
      "border border-line-strong text-ink-primary hover:border-signal-cyan/50 hover:text-signal-cyan",
...
export function PgInput(props: InputHTMLAttributes<HTMLInputElement>) {
  return (
    <input
      {...props}
      className={`w-full rounded-lg border border-line-subtle bg-void px-3 py-2 font-mono text-sm text-ink-primary placeholder:text-ink-tertiary focus:border-signal-cyan/50 focus:outline-none ${props.className ?? ""}`}
    />
  );
}
...
export function PgSelect({
  ...
  return (
    <select
      ...
      className={`w-full rounded-lg border border-line-subtle bg-void px-3 py-2 font-mono text-sm text-ink-primary focus:border-signal-cyan/50 focus:outline-none ${className ?? ""}`}
    >
      {options.map((o) => (
  ...
  return (
    <label className="block">
      <div className="mb-1.5 flex justify-between font-mono text-xs text-ink-secondary">
        <span>{label}</span>
        <span className="text-signal-cyan">{format ? format(value) : value}</span>
      </div>
      <input
        type="range"
        min={min}
        ...
        className="h-1.5 w-full cursor-pointer appearance-none rounded-full bg-void-card accent-signal-cyan"
      />
    </label>
  );
}
...
export function PgToggle({
  ...
  return (
    ...
    >
      ...
      <span
        className={`relative h-5 w-9 shrink-0 rounded-full transition-colors ${checked ? "bg-signal-cyan" : "bg-line-strong"}`}
      >
        <motion.span
    ...
    <div className="flex flex-wrap items-center gap-2">
      ...
      {scenarios.map((s) => (
        <button
          key={s.label}
          onClick={s.onClick}
          className="rounded-full border border-line-subtle px-3 py-1 font-mono text-[11px] text-ink-secondary transition-colors hover:border-signal-cyan/50 hover:text-signal-cyan"
        >
          {s.label}
        </button>
      ))}
